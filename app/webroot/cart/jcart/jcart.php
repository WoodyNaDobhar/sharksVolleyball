...
class Jcart {
	...
	*/
	public function display_cart() {
		...
		$jcartToken = (isset($jcartToken) && isset($_POST[$jcartToken]))?$_POST[$jcartToken]:'';
		...
		if ($isCheckout === true) {
			...
			$checkout = $path . '/jcart/gateway.php';
		...
		echo tab(1) . "<form method='post' action='$checkout' $target>\n";
		echo tab(2) . "<fieldset>\n";
		...
		if ($isCheckout !== true) {
			...
			echo tab(7) . "<input type='$inputType' ".(isset($src)?$src:'')." id='jcart-checkout' name='jcartCheckout' class='jcart-button' value='{$config['text']['checkout']}' />\n";
		}
		...
		echo tab(4) . "<input type='$inputType' ".(isset($src)?$src:'')." name='jcartUpdateCart' value='{$config['text']['update']}' class='jcart-button' />\n";
		...
		echo tab(4) . "<input type='$inputType' ".(isset($src)?$src:'')." name='jcartEmpty' value='{$config['text']['emptyButton']}' class='jcart-button' />\n";
		echo tab(3) . "</div>\n";
		...
		if ($isCheckout === true) {
			...
			echo tab(3) . "<input type='$inputType' ".(isset($src)?$src:'')." id='jcart-paypal-checkout' name='jcartPaypalCheckout' value='{$config['text']['checkoutPaypal']}' ".(isset($disablePaypalCheckout)?$disablePaypalCheckout:'')." />\n";
		}
